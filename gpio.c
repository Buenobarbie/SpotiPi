...
void gpio_put(unsigned gpio, int valor) {
  ...
  if (valor) {
    if (gpio < 32)
      GPIO_REG(gpset[0]) = (1 << gpio);
    else
      GPIO_REG(gpset[1]) = (1 << (gpio - 32));
  } else {
    if (gpio < 32)
      GPIO_REG(gpclr[0]) = (1 << gpio);
    else
      GPIO_REG(gpclr[1]) = (1 << (gpio - 32));
  }
}
...
int gpio_get(unsigned gpio) {
  if (gpio > 53)
    return 0;
  if (gpio < 32)
    return GPIO_REG(gplev[0]) & (1 << gpio);
  else
    return GPIO_REG(gplev[1]) & (1 << (gpio - 32));
}
