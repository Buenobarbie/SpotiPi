...
void pwm_start(float div) {
  ...
  uint32_t divf = (uint32_t)(frac_part * 4096); //multiplica por 2¹² para tirar os 12 primeiros bits da parte fracionária
...
void pwm_config(unsigned channel) {
  ...
  if (channel == 0) {
    ...
    // não será) Bit 3 (SBIT1):      0 → Sem uso (bit de silêncio), manter 0 Bit
    ...
    // Bit 7 (MSEN1):      0→ Utiliza o algorítmo explicitado na documentação 1
    ...
    // Bit 10 (RPTL2):      0 → Ignorado (só tem efeito se FIFO for usada, que
    // não será) Bit 11 (SBIT2):      0 → Sem uso (bit de silêncio), manter 0
    // Bit 12 (POLA2):      0 → Polaridade normal (nível alto durante pulso
    // ativo) Bit 13 (USEF2):      0 → FIFO não será usada Bit 14 (reserved): 0
...
/* void pwm_set_polarity(int channel, int polarity) {
  ...
  if (channel == 0) {
  PWM_REG(ctl) = (PWM_REG(ctl) & ~0b0001000000) | (polarity << 4);
  }
  else {
